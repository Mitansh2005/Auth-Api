...
from .serializers import UserRegisterationSerializer,CustomUserLoginSerializer,UserProfileSerializer,UserChangePasswordSerializer,UserChangePasswordEmailSerializer
...
class UserLogin(APIView):
   ...
   def post(self,request,format=None):
      ...
      if serializer.is_valid(raise_exception=True):
         ...
         if user is not None:
          token=get_tokens_for_user(user)
          return Response({'token':token,'msg':'login was successful'},status=status.HTTP_201_CREATED)
         else:
